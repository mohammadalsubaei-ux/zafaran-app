...
export default function CartScreen() {
  const { items, updateQty, removeItem, clearCart, total, totalItems, chef_id } = useCart();
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.title}>سلتي 🛍️</Text>
      ...
      <View style={s.footer}>
        <TouchableOpacity style={s.orderBtn} onPress={handleOrder} disabled={loading}>
          {loading
            ? <ActivityIndicator color="#0E0700" />
            : <Text style={s.orderBtnText}>🛍️ اطلب الآن — {grandTotal} ريال</Text>
          }
        </TouchableOpacity>
      </View>
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  itemName:          { fontSize: 14, fontWeight: "800", color: "#FDF0DC", textAlign: "right" },
  itemPrice:         { fontSize: 13, color: "#F0A500", fontWeight: "700", textAlign: "right", marginTop: 3 },
  ...
  qtyNum:            { fontSize: 16, fontWeight: "900", color: "#FDF0DC", minWidth: 20, textAlign: "center" },
  section:           { backgroundColor: "#1C1000", borderRadius: 16, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: "rgba(240,165,0,0.1)" },
  sectionTitle:      { fontSize: 14, fontWeight: "800", color: "#FDF0DC", textAlign: "right", marginBottom: 12 },
  ...
  deliveryLabel:     { fontSize: 13, fontWeight: "700", color: "#8A6030" },
  deliveryLabelActive: { color: "#F0A500" },
  deliveryPrice:     { fontSize: 11, color: "#5A3A18", marginTop: 2 },
  locBtn:            { backgroundColor: "#F0A500", borderRadius: 12, padding: 12, alignItems: "center", marginBottom: 8 },
  locBtnText:        { fontSize: 14, fontWeight: "800", color: "#0E0700" },
  addressText:       { color: "#4CAF50", fontSize: 12, textAlign: "right", fontWeight: "700" },
  summaryRow:        { flexDirection: "row-reverse", justifyContent: "space-between", marginBottom: 8 },
  summaryLabel:      { fontSize: 13, color: "#8A6030" },
  summaryVal:        { fontSize: 13, color: "#FDF0DC", fontWeight: "700" },
  totalRow:          { borderTopWidth: 1, borderTopColor: "rgba(240,165,0,0.12)", paddingTop: 10, marginTop: 4 },
  totalLabel:        { fontSize: 15, fontWeight: "800", color: "#FDF0DC" },
  totalVal:          { fontSize: 18, fontWeight: "900", color: "#F0A500" },
  ...
  orderBtnText:      { fontSize: 17, fontWeight: "900", color: "#0E0700" },
});