...
export default function OrderDetailScreen() {
  ...
  useEffect(() => {
    fetch(`${API}/api/orders/${id}`)
      .then(r => r.json())
      .then(j => { if (j.success) setOrder(j.data); })
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={s.card}>
          ...
          <Text style={s.date}>{new Date(order.created_at).toLocaleDateString("ar-SA", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</Text>
        </View>
        ...
        {/* الشيف */}
        <View style={s.card}>
          <Text style={s.cardTitle}>الشيف 👨‍🍳</Text>
          <Text style={s.chefName}>{isMale ? "👨‍🍳" : "👩‍🍳"} {order.chefs?.users?.full_name}</Text>
          <Text style={s.chefCity}>📍 {order.chefs?.city} · {order.chefs?.neighborhood}</Text>
        </View>
          ...
          {orderItems.map((oi: any) => (
            <View key={oi.id} style={s.itemRow}>
              ...
              <View style={s.itemRight}>
                ...
                <Text style={s.itemPrice}>{oi.subtotal || oi.price * oi.quantity} ريال</Text>
              </View>
            </View>
          ))}
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.summaryRow}>
            ...
            <Text style={s.summaryVal}>{order.subtotal} ريال</Text>
          </View>
          <View style={s.summaryRow}>
            <Text style={s.summaryLabel}>التوصيل</Text>
            <Text style={s.summaryVal}>{order.delivery_fee} ريال</Text>
          </View>
          {order.distance_km > 0 && (
            <View style={s.summaryRow}>
          ...
          <View style={[s.summaryRow, s.totalRow]}>
            ...
            <Text style={s.totalVal}>{order.total} ريال</Text>
          </View>
        </View>
