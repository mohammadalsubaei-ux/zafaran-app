...
export default function LoginScreen() {
  ...
  const handleRegister = async (role: string) => {
    ...
    if ((role === "chef" || role === "driver") && !city.trim()) { Alert.alert("تنبيه", "أدخل مدينتك"); return; }
    setLoading(true);
    try {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={s.scroll}>
            ...
            <View style={s.form}>
              ...
              <TouchableOpacity style={[s.chefBtn, { marginTop: 10, borderColor: "rgba(33,150,243,0.3)", backgroundColor: "rgba(33,150,243,0.08)" }]} onPress={() => setStep("driver_register")}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <View style={s.form}>
            ...
            <Text style={s.formHint}>وصّل الطلبات واكسب أكثر</Text>
            <Text style={s.label}>الاسم</Text>
            <View style={s.inputWrap}>
              <TextInput style={s.input} placeholder="اسمك الكامل" placeholderTextColor="#5A3A18" value={name} onChangeText={setName} textAlign="right"/>
            </View>
...
const s = StyleSheet.create({
  ...
  genderBtn:      { flex: 1, alignItems: "center", backgroundColor: "#251400", borderRadius: 14, paddingVertical: 12, borderWidth: 1, borderColor: "rgba(240,165,0,0.1)", gap: 4 },
