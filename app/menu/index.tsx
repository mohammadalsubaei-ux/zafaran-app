...
export default function MenuScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem("user").then(async u => {
      ...
      try {
        ...
        setLoading(false);
        Alert.alert("خطأ", "تعذر تحميل بيانات الطباخ");
      }
    });
  }, []);
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      Alert.alert("خطأ", "تعذر تحميل الوجبات");
    } finally {
      setLoading(false);
    }
  };
...
setPrepMinutes(String(totalMinutes % 60));
  ...
  const saveItem = async () => {
    if (!chefId) {
      Alert.alert("خطأ", "لم يتم العثور على حساب الطباخ");
      return;
    }

    if (!name.trim()) {
      Alert.alert("تنبيه", "أدخل اسم الوجبة");
      return;
    }

    if (!price || isNaN(Number(price))) {
      Alert.alert("تنبيه", "أدخل سعر صحيح");
      return;
    }
...
prep_minutes:
  status === "preorder"
    ...
    : 0,
      ...
      const res = await fetch(
        editItem ? `${API}/api/menu/${editItem.id}` : `${API}/api/menu`,
        {
      ...
      if (json.success) {
        ...
        Alert.alert("خطأ", json.message || "لم تتم العملية");
      }
    } catch {
      ...
      { text: "لا", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("خطأ", "تعذر حذف الوجبة");
          }
        },
      },
    ]);
  ...
  const toggleItemStatus = async (item: any) => {
    ...
    const newStatus =
      current === "available" ? "preorder" :
      current === "preorder" ? "unavailable" :
      "available";
    ...
    try {
      await fetch(`${API}/api/menu/${item.id}`, {
        ...
        body: JSON.stringify({
          status: newStatus,
          is_available: newStatus !== "unavailable",
        }),
      });

      loadItems();
    } catch {
      Alert.alert("خطأ", "تعذر تغيير حالة الوجبة");
    }
  };
                ...
                <View style={s.cardInfo}>
                  ...
                  {item.status === "preorder" && item.prep_hours > 0 && (
                    <Text style={s.prepTime}>
                      ⏱️ وقت التحضير: {item.prep_hours} ساعة
                    </Text>
                  )}
                </View>
                ...
                <View style={s.cardRight}>
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor:
                          STATUS_COLORS[item.status || "available"] + "22",
                      },
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        s.statusBadgeText,
                        { color: STATUS_COLORS[item.status || "available"] },
                      ]}
                    >
            ...
            {ITEM_STATUS.map(st => (
              <TouchableOpacity
                ...
                style={[
                  s.statusOption,
                  status === st.id && {
                    borderColor: STATUS_COLORS[st.id],
                    backgroundColor: STATUS_COLORS[st.id] + "11",
                  },
                ]}
                onPress={() => setStatus(st.id)}
              >
                <View>
                  <Text style={[s.statusLabel, { color: STATUS_COLORS[st.id] }]}>
                    {st.label}
                  </Text>
                ...
                {status === st.id && (
                  <Text style={{ color: STATUS_COLORS[st.id], fontSize: 18 }}>✓</Text>
                )}
              </TouchableOpacity>
            ))}
            ...
            <TouchableOpacity style={s.saveBtn} onPress={saveItem} disabled={saving}>
              {saving ? (
                ...
                <Text style={s.saveBtnText}>
                  {editItem ? "حفظ التعديلات ✅" : "إضافة الوجبة ✅"}
                </Text>
              )}
            </TouchableOpacity>
          </ScrollView>
